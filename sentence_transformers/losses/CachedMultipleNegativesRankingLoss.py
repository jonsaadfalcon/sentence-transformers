    ...
    def embed_minibatch(
        ...
        with random_state_context:
            with grad_context():
                ...
                if self.model.max_seq_length in [512]:
                    reps = self.model(sentence_feature_minibatch)["sentence_embedding"]  # (mbsz, hdim)
                else:
                    reps = self.model[0]._modules['auto_model'](input_ids=sentence_feature_minibatch['input_ids'], 
                                                                attention_mask=sentence_feature_minibatch['attention_mask'], 
                                                                token_type_ids=sentence_feature_minibatch['token_type_ids'])["sentence_embedding"]  # (mbsz, hdim)
