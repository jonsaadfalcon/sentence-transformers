...
class SentenceTransformer(nn.Sequential):
    ...
    #    Tokenizes the texts
    #    """
    #    #return self._first_module().tokenize(texts, padding=True, truncation=True)
    ...
    def tokenize(self, texts: Union[List[str], List[Dict], List[Tuple[str, str]]]):
        ...
        def expand_tensor(input_tensor):
            target_size = (1, self._first_module().tokenizer.model_max_length)
        ...
        starting_tensor = self._first_module().tokenize(texts)
        
        new_starting_tensor = {}
        new_starting_tensor['input_ids'] = expand_tensor(starting_tensor['input_ids'].clone()).clone()[:self._first_module().tokenizer.model_max_length]
        new_starting_tensor['attention_mask'] = expand_tensor(starting_tensor['attention_mask'].clone()).clone()[:self._first_module().tokenizer.model_max_length]
        new_starting_tensor['token_type_ids'] = expand_tensor(starting_tensor['token_type_ids'].clone()).clone()[:self._first_module().tokenizer.model_max_length]
        
        return new_starting_tensor
    ...
    def save(self, path: str, global_step:int = None, model_name: Optional[str] = None, create_model_card: bool = True, train_datasets: Optional[List[str]] = None):
        """
        ...
        for idx, name in enumerate(self._modules):
            ...
            try:
                ...
                model_path = model_path + f"checkpoint_{global_step}.pt"
                torch.save(self._modules['0'].auto_model._modules['module'].state_dict(), model_path)
                print("Intermediate Checkpoint: " + model_path)
            ###
            #module._modules['auto_model']._modules['module'].save(model_path)
            ###
    ...
    def fit(self,
        ...
        print("Target Device in Sentence Transformers: " + str(self._target_device))
        ...
        for epoch in trange(epochs, desc="Epoch", disable=not show_progress_bar):
            ...
            for item_number in trange(steps_per_epoch, desc="Iteration", smoothing=0.05, disable=not show_progress_bar):
                for train_idx in range(num_train_objectives):
                    ...
                    if use_amp:
                        ...
                        with torch.autograd.set_detect_anomaly(True):
                            loss_value = loss_model(features, labels)
                            loss_value.backward()
                            torch.nn.utils.clip_grad_norm_(loss_model.parameters(), max_grad_norm, error_if_nonfinite=True)
    ...
    def _save_checkpoint(self, checkpoint_path, checkpoint_save_total_limit, step):
        ...
        self.save(os.path.join(checkpoint_path, str(step)), global_step=step)
